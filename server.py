...
class Server:
    ...
    def __init__(self, IP, PORT=5555):
        ...
        self.translator = asl_translator.ASLTranslator()
                    ...
                    if username is None:  # Client not authenticated
                        ...
                        match parsed_req[0]:
                            case 'REG':  # Registration
                                ret_code = self.register_user(username=parsed_req[1], password=parsed_req[2])
                                print("here")
                                print(ret_code)
                                # if ret_code:
                            ...
                            case _:  # Error: Unauthenticated action
                                protocol.server_protocol.send_error(cl_socket=cl_socket, aes_key=aes_key)#, error_msg="Error: Client is not logged in!")
                                continue
                        ...
                        match parsed_req[0]:
                            case 'REQ': # Client requests a translation
                                ...
                                if data:
                                    with self.lock:
                                        ...
                                        asl_translation = self.translator.translate(data)
                                        print(f"[*] Translation: {asl_translation} [*]")
                                        protocol.server_protocol.send_msg(aes_key=aes_key, socket=cl_socket, data=asl_translation.encode(encoding="latin-1"))
                                        continue
                                else:
                            ...
                            case _:  # Unknown request
                                protocol.server_protocol.send_error( cl_socket=cl_socket, aes_key=aes_key)#, error_msg="Error: Unknown request.")
                                break
